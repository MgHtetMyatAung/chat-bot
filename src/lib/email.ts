...
const FROM_EMAIL = "Nexus AI <onboarding@resend.dev>"; // Resend's default testing email
...
export const sendAuthEmail = {
    verification: async (email: string, url: string) => {
        const { data, error } = await resend.emails.send({
            from: FROM_EMAIL,
            to: [email],
            subject: 'Verify your email address - Nexus AI',
            html: `
                <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; color: #111;">
                    <h1 style="color: #2563eb;">Welcome to Nexus AI!</h1>
